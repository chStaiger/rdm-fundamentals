...
Below we are capturing some fictive metadata as key-value pairs.
...
In the image below the *same metadata* is expressed as a network of relationships, also known as Knowledge Graph.
...
To serialise the graph we use the format `turtle` in this example:
...
ex:dataset1
    ...
    dc:creator ex:personAnna ;
    dc:subject ex:conceptSoilMoisture .

# Person node
ex:personAnna
    schema:name "Anna Vermeer" ;
    schema:affiliation ex:orgUtrecht .
...
# Concept node
ex:conceptSoilMoisture
    rdfs:label "Soil Moisture" .
```

We go more into detail into metadata serialisation and infrastructure in the Chapter [Metadata Infrastructure](metadata-portals.qmd).
